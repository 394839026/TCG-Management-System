...
import { BookOpen, Plus, Heart, Eye, Trash2, Copy } from 'lucide-react'
...
export function DeckCollectionPage() {
  ...
  const { data: decksData, isLoading } = useQuery({
    queryKey: ['decks', 'collection', activeTab],
  ...
  const copyMutation = useMutation({
    mutationFn: (id: string) => deckService.getById(id),
    onSuccess: (data) => {
      if (data?.data) {
        const originalDeck = data.data
        const copiedDeck = {
          ...originalDeck,
          _id: undefined,
        ...
        setEditingDeck(copiedDeck as Deck)
        setFormOpen(true)
        toast.success('卡组已复制，现在可以编辑')
      }
    },
    onError: () => {
      toast.error('复制失败')
  ...
  const decks: Deck[] = (decksData?.data || []).filter((deck: Deck) => deck.type === 'deck' || !deck.type)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">管理和分享你的卡组</p>
        </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge variant="default" className="bg-blue-600">卡组</Badge>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    {activeTab === 'my' ? (
                      ...
                      <>
                        <Button size="sm" className="flex-1" onClick={() => navigate(`/decks/${deck._id}`)}>查看</Button>
                        <Button variant="outline" size="sm" className="flex-1" onClick={() => copyMutation.mutate(deck._id)}>
